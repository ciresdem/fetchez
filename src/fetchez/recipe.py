...
class Recipe:
    ...
    def _generate_receipt(self):
        ...
        for hook in self.config.get("global_hooks", []):
            ...
            # Dynamically hunt for declared output files!
            args = hook.get("args", {})
            if "output" in args:
                expected_outputs.append(args["output"])
        ...
        with open(receipt_path, "w", encoding="utf-8") as f:
            f.write(f"# DEM Generation Receipt: {name}\n")
            f.write("**Generated by Globato/Fetchez**\n\n")
            f.write("## Project Description\n")
            f.write(f"{desc}\n\n")
            f.write("## Bounding Box\n")
            f.write(f"`{region}`\n\n")
            f.write("## Data Sources Fetched\n")
            f.write("\n".join(modules_info) + "\n\n")
            f.write("## Global Processing Pipeline\n")
            f.write("\n".join(hooks_info) + "\n")
