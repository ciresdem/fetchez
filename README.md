...
* **Strict Execution Lifecycle:** Fetchez formally separates data engineering into distinct phases (`manifest` -> `file` -> `stream` -> `collection`). This guarantees that file unzipping, in-memory point filtering, and final raster creation always happen in the correct order.
